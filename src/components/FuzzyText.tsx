...
const FuzzyText: React.FC<FuzzyTextProps> = ({
  ...
  useEffect(() => {
    ...
    const fontString = `${fontWeight} ${fontSizeStr} '${fontFamily}'`; // 注意加单引号
    ...
    await document.fonts.ready;
      ...
      const xOffset = 0; // offscreen canvas 内直接从左绘制
      offCtx.font = fontString;
      offCtx.textBaseline = 'alphabetic';
      ...
      // **关键修改**: 居中 offset
      const centeredXOffset = (canvas.width - offscreenWidth) / 2 - horizontalMargin;

      const interactiveLeft = horizontalMargin + centeredXOffset;
      ...
      const run = (timestamp: number) => {
        ...
        for (let j = 0; j < tightHeight; j++) {
          ...
          ctx.drawImage(offscreen, centeredXOffset, j, offscreenWidth, 1, dx, j + dy, offscreenWidth, 1);
        }
